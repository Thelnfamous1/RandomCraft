...
public class CraftingMenuMixin {
    ...
    private static ItemStack randomcraft$handleRecipeAssemble(CraftingRecipe original, Container argContainer, Operation<ItemStack> operation,
        ...
        if(pLevel.getGameRules().getBoolean(RandomCraft.RULE_RANDOM_CRAFTING.get())){
            ...
            if(recipePairing != null){
                Optional<? extends Recipe<?>> alternateRecipe = pLevel.getServer().getRecipeManager().byKey(recipePairing);
                Optional<ItemStack> alternateResult = alternateRecipe
                        .map(r -> {
                            try {
                ...
                ((AlternateRecipeHolder)pResult).randomcraft$setAlternateRecipeUsed(alternateRecipe.orElse(null));
                return alternateResult.orElse(operation.call(original, argContainer));
            }
        }
